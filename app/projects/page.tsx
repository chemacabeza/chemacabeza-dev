...
const githubProjects = [
    {
        ...
        description:
            "\"At your service, sir!\" A production-ready, full-stack AI butler that securely connects to multiple Google Accounts via OAuth 2.0 to manage your Gmail, Drive, Calendar, and Maps concurrently — and now connects natively to your WhatsApp via a custom Dockerized bridge (Baileys), with no Meta API tokens required. Simply scan a QR code in the high-fidelity dark-mode dashboard (mirroring WhatsApp Web) to link your personal account. Recently updated with a \"nuclear\" atomic session reset to purge stale connections instantly. The AI assistant is fully bilingual (English and Spanish), capable of aggregating and searching across multiple Gmail inboxes and Drive accounts, sending WhatsApp messages on your behalf, deleting calendar events, and automatically generating Android Auto navigation links for any address in your schedule. Powered by OpenAI for intelligent intent parsing, it features custom email reply templates, geocoding via Google Maps, and AES-encrypted token storage at rest. Get started in 5 commands: git clone → ./build.sh → scan QR → Alfred is live at localhost:5173. Fully containerized: Spring Boot 3.4 + React 18 + Node.js Bridge + PostgreSQL 16.",
