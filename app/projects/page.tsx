...
const githubProjects = [
    {
        ...
        subtitle: "Alfred · OpenAI · Bilingual (EN/ES) · WhatsApp · Android Auto · Public Transport · Gmail, Calendar & Maps",
        description:
            "\"At your service, sir!\" A production-ready, full-stack AI butler that securely connects to your Google Account via OAuth 2.0 to manage Gmail, Calendar, and Maps — and now connects to your WhatsApp via the Meta Cloud API. Converses fluently in both English and Spanish. The WhatsApp integration features a high-fidelity dark-mode dashboard mirroring your conversations in real time via webhooks, and the AI assistant can send WhatsApp messages on your behalf when you ask. Also featuring native Android Auto and public transport integration for hands-free assistance while commuting by car or train. Powered by OpenAI for intelligent intent parsing and natural-language responses, it features custom email reply templates, calendar event scheduling, geocoding via Google Maps, and AES-encrypted token storage at rest. Get started in 5 commands: git clone → cp .env.example .env → add your API keys → ./build.sh → ./start.sh → Alfred is live at localhost:5173. Requires 5 APIs: Google OAuth 2.0 Client ID & Secret, Gmail API, Google Calendar API, Google Maps API, and an OpenAI API Key. WhatsApp requires a Meta Business App with webhook configuration. Fully containerized: Spring Boot 3 backend + React 18/Vite frontend + PostgreSQL 16 — all orchestrated with a single Docker Compose command.",
        tags: ["Spring Boot 3", "React 18", "OpenAI", "WhatsApp Cloud API", "Bilingual", "Android Auto", "Public Transport", "Google OAuth2", "Gmail API", "Calendar API", "Maps API", "PostgreSQL", "Docker"],
    },
    {
        repo: "test-with-llms",
