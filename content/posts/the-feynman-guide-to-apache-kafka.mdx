...
* **Retention Policy:** Unlike message queues that delete messages after they are read, Kafka persists events. You configure how long to keep them (e.g., 7 days, or forever) based on time or size.
...
## Running a Local Kafka Environment (Docker Compose)

Ready to start building? Save the following `docker-compose.yml` to spin up a local single-broker KRaft Kafka environment alongside a modern Web UI console (`provectus-labs/kafka-ui`) for exploring topics, partitions, and messages:
...
docker compose up -d
```
Then navigate to `http://localhost:8080` in your browser to view your topics, configure partitions, inspect incoming messages, and monitor consumer offsets.
