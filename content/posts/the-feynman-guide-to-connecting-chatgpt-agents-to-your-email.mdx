...
`"Create an Hourly Change Reporter agent that audits my Gmail activity for sometest@gmail.com, creates a structured change table with impact analysis, and emails the report to sometest@gmail.com."`
...
*The complete email agent loop: Activity query -> Data Triage & State Comparison -> Change Log Formatting -> Report Generation -> Automated Dispatch to sometest@gmail.com.*
...
> While automated shift reports can be sent directly to your own personal address (`sometest@gmail.com`), any outbound email sent to *third parties* should always be written to `/drafts` for manual human review.
