...
## 7. Tying It All Together: Docker Compose (The Building Manager)
...
Here is what a real-world Docker Compose file looks like, bringing together everything we've learned—Images, Ports, Volumes, and Networks:
...
services:
  # The Web Application
  web:
    build: .                           # Use the local Dockerfile (Blueprint)
    ports:
      - "8080:80"                      # Port Mapping (Building buzzer routing)
    depends_on:
      - database                       # Wait for the database to start
    networks:
      - app-network                    # Bridge Network (Apartment Intercom)

  # The Database
  database:
    image: postgres:15                 # Use a pre-built Blueprint from Docker Hub
    environment:
      POSTGRES_USER: admin
      POSTGRES_PASSWORD: secretpassword
    volumes:
      - db-data:/var/lib/postgresql/data # Volume (External Hard Drive) for data persistence
    networks:
      - app-network                    # Bridge Network (Apartment Intercom)

# Define the Volumes and Networks explicitly
volumes:
  db-data:                             # Declare the external hard drive

networks:
  app-network:                         # Declare the intercom system
```

With this single file, you can bring up your entire infrastructure with one command:

```bash
docker-compose up -d
```

And just like that, your code works identically on your machine, your coworker's laptop, and the production server.
