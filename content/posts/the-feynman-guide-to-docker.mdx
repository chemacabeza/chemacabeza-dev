...
* With Multi-Stage builds, you use multiple `FROM` instructions in a single `Dockerfile`. You compile your code in a temporary "build stage" containing all the compilers, and then copy *only* the compiled artifact into a final, clean, lightweight "production stage."
...
* **Analogy:** Imagine you have a custom container that wraps the `git` command. 
  - You set the `ENTRYPOINT` to `["git"]`.
  - You set the `CMD` to `["status"]`.
  - If you run the container normally, it runs `git status`.
  - But if you run it with an argument (e.g., `docker run my-git-app log`), Docker overrides the `CMD` (arguments) with `log`, running `git log`. The tool (`ENTRYPOINT`) remains `git`, but the subcommand (`CMD`) was easily changed!
...
* To secure your container, always create and switch to a non-root user in your `Dockerfile` after executing commands that require admin permissions (like installing system packages).
